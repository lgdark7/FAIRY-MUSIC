...
sealed class ListenTogetherEvent {
  ...
  data class JoinApproved(val roomCode: String, val userId: String, val state: RoomState) :
    ListenTogetherEvent()
  ...
  data class LocalSuggestionApproved(val payload: SuggestionReceivedPayload) :
    ListenTogetherEvent()
...
@Singleton
class ListenTogetherClient @Inject constructor(private val context: Context) {
  companion object {
  ...
  val pendingSuggestions: StateFlow<List<SuggestionReceivedPayload>> =
    _pendingSuggestions.asStateFlow()
  ...
  private val _events =
    MutableSharedFlow<ListenTogetherEvent>(
  ...
  private fun loadBlockedUsernames() {
    try {
      val blockedJson =
        context.dataStore.get(echo.music.iad1tya.constants.ListenTogetherBlockedUsersKey, "")
      val blockedList =
  ...
  private fun executePendingAction() {
    ...
    when (action) {
      ...
      is PendingAction.JoinRoom -> {
        log(
          LogLevel.INFO,
          "Executing pending join room",
          "${action.roomCode} as ${action.username}"
        )
        sendMessage(
          MessageTypes.JOIN_ROOM,
          JoinRoomPayload(action.roomCode.uppercase(), action.username)
        )
      }
    }
  }
  ...
  private fun showSuggestionNotification(payload: SuggestionReceivedPayload) {
    ...
    val content =
      context.getString(
        R.string.listen_together_suggestion_received,
    ...
    try {
      ...
      when (msgType) {
        ...
        MessageTypes.JOIN_REQUEST -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? JoinRequestPayload
              ?: return

          if (isUserBlocked(payload.username)) {
            log(
              LogLevel.INFO,
              "Join request from blocked user ignored",
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.JoinRequestReceived(payload.userId, payload.username))
          }
        }
        ...
        MessageTypes.USER_JOINED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? UserJoinedPayload
              ?: return
          _roomState.value =
          ...
          _pendingJoinRequests.value =
            _pendingJoinRequests.value.filter { it.userId != payload.userId }
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.UserJoined(payload.userId, payload.username))
          }
        }
        MessageTypes.USER_LEFT -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? UserLeftPayload ?: return
          _roomState.value =
            _roomState.value?.copy(
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.UserLeft(payload.userId, payload.username))
          }
        }
        MessageTypes.HOST_CHANGED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? HostChangedPayload
              ?: return
          _roomState.value =
            _roomState.value?.copy(
              hostId = payload.newHostId,
              users =
                _roomState.value!!.users.map { it.copy(isHost = it.userId == payload.newHostId) }
            )
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.HostChanged(payload.newHostId, payload.newHostName))
          }
        }
        MessageTypes.KICKED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? KickedPayload ?: return
        ...
        MessageTypes.SYNC_PLAYBACK -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? PlaybackActionPayload
              ?: return
          ...
          when (payload.action) {
            ...
            PlaybackActions.SEEK -> {
              _roomState.value =
                _roomState.value?.copy(position = payload.position ?: _roomState.value!!.position)
            }
            ...
            PlaybackActions.QUEUE_ADD -> {
              ...
              if (ti != null) {
                ...
                _roomState.value =
                  _roomState.value?.copy(
                    queue =
                      if (payload.insertNext == true) listOf(ti) + currentQueue
                      else currentQueue + ti
                  )
              }
            }
            PlaybackActions.QUEUE_REMOVE -> {
              ...
              if (!id.isNullOrEmpty()) {
                val currentQueue = _roomState.value?.queue ?: emptyList()
                _roomState.value =
                  _roomState.value?.copy(queue = currentQueue.filter { it.id != id })
              }
            }
        ...
        MessageTypes.BUFFER_WAIT -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? BufferWaitPayload
              ?: return
          _bufferingUsers.value = payload.waitingFor
          log(LogLevel.DEBUG, "Waiting for buffering", "Users: ${payload.waitingFor.size}")
          scope.launch {
            _events.emit(ListenTogetherEvent.BufferWait(payload.trackId, payload.waitingFor))
          }
        }
        MessageTypes.BUFFER_COMPLETE -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? BufferCompletePayload
              ?: return
          _bufferingUsers.value = emptyList()
        ...
        MessageTypes.SYNC_STATE -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? SyncStatePayload
              ?: return
          log(
            LogLevel.INFO,
            "Sync state received",
            "Playing: ${payload.isPlaying}, Position: ${payload.position}"
          )
          scope.launch { _events.emit(ListenTogetherEvent.SyncStateReceived(payload)) }
        }
        MessageTypes.SUGGESTION_RECEIVED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? SuggestionReceivedPayload
              ?: return
          ...
          if (_role.value == RoomRole.HOST) {
            ...
            if (isUserBlocked(payload.fromUsername)) {
              log(
                LogLevel.INFO,
                "Suggestion from blocked user ignored",
        ...
        MessageTypes.SUGGESTION_APPROVED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? SuggestionApprovedPayload
              ?: return
        ...
        MessageTypes.SUGGESTION_REJECTED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? SuggestionRejectedPayload
              ?: return
        ...
        MessageTypes.ERROR -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? ErrorPayload ?: return
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.ServerError(payload.code, payload.message))
          }
        }
        MessageTypes.PONG -> {
          if (pingSentTime > 0) {
        ...
        MessageTypes.RECONNECTED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? ReconnectedPayload
              ?: return
          _userId.value = payload.userId
        ...
        MessageTypes.USER_RECONNECTED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? UserReconnectedPayload
              ?: return
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.UserReconnected(payload.userId, payload.username))
          }
        }
        MessageTypes.USER_DISCONNECTED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? UserDisconnectedPayload
              ?: return
          ...
          scope.launch {
            _events.emit(ListenTogetherEvent.UserDisconnected(payload.userId, payload.username))
          }
        }
        MessageTypes.CHAT -> {
          var payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? ChatMessagePayload
              ?: return
        ...
        MessageTypes.ROOM_SETTINGS_CHANGED -> {
          val payload =
            codec.decodePayload(msgType, payloadBytes, detectedFormat) as? UpdateRoomSettingsPayload
              ?: return
          _roomState.value =
            _roomState.value?.copy(allowParticipantControl = payload.allowParticipantControl)
          log(
            LogLevel.INFO,
  ...
  fun suggestTrack(trackInfo: TrackInfo) {
    ...
    scope.launch(Dispatchers.Main) {
      Toast.makeText(
          context,
          context.getString(R.string.listen_together_suggestion_sent),
          Toast.LENGTH_SHORT
        )
        .show()
    }
  ...
  fun blockUser(username: String) {
    ...
    _pendingJoinRequests.value =
      _pendingJoinRequests.value.filter { it.username !in _blockedUsernames.value }
    _pendingSuggestions.value =
      _pendingSuggestions.value.filter { it.fromUsername !in _blockedUsernames.value }

    scope.launch { saveBlockedUsernames() }
