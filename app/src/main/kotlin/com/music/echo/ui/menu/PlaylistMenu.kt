...
fun PlaylistMenu(
  ...
  val isSignedIn = remember(innerTubeCookie) { "SAPISID" in parseCookieString(innerTubeCookie) }
  ...
  LaunchedEffect(Unit) {
    if (autoPlaylist == false) {
      database.playlistSongs(playlist.id).collect { songs = it.map(PlaylistSong::song) }
    } else {
      if (songList != null) {
        songs = songList
      }
  ...
  val isExporting =
    remember(exportingSongIds, songs) { songs.any { exportingSongIds.split(",").contains(it.id) } }
  val isExported =
    remember(exportedSongIds, songs) {
      songs.isNotEmpty() && songs.all { exportedSongIds.split(",").contains(it.id) }
    }
          ...
          onClick = {
            ...
            if (exportDirectoryUri.isBlank()) {
              android.widget.Toast.makeText(
                  context,
                  R.string.export_directory_not_set,
  ...
  PlaylistListItem(
    ...
    trailingContent = {
      if (playlist.playlist.isEditable != true) {
        IconButton(
          onClick = { database.query { dbPlaylist?.playlist?.toggleLike()?.let { update(it) } } }
        ) {
          Icon(
            painter =
              painterResource(
            ...
            tint =
              if (dbPlaylist?.playlist?.bookmarkedAt != null) MaterialTheme.colorScheme.error
              ...
              onClick = {
                ...
                val intent =
                  Intent().apply {
                    ...
                    putExtra(
                      Intent.EXTRA_TEXT,
                      "https://youtube.com/playlist?list=${dbPlaylist?.playlist?.browseId}"
                    )
                  }
            ...
            add(
              Material3MenuItemData(
                title = {
                  Text(
                    text =
                      if (playlist.playlist.isPinned) stringResource(R.string.unpin_playlist)
                      else stringResource(R.string.pin_playlist)
                  )
                },
                icon = {
                  Icon(
                    painter =
                      painterResource(
                        if (playlist.playlist.isPinned) R.drawable.ic_push_pin
                  ...
                  onClick = {
                    ...
                    coroutineScope.launch(Dispatchers.IO) {
                      try {
                        ...
                        if (browseId != null) {
                          database.query { update(playlist.playlist.copy(browseId = browseId)) }
                          var successCount = 0
                          songs.forEachIndexed { index, song ->
                            ...
                            withContext(Dispatchers.Main) { syncedCount = index + 1 }
                          ...
                          kotlinx.coroutines.withContext(Dispatchers.Main) {
                            if (successCount == songs.size) {
                              ...
                              Toast.makeText(
                                  context,
                                  context.getString(
                                    R.string.sync_completed_partial,
                                    successCount,
                                    songs.size
                                  ),
                                  Toast.LENGTH_LONG
                        ...
                        kotlinx.coroutines.withContext(Dispatchers.Main) {
                          Toast.makeText(context, e.message ?: "Failed to sync", Toast.LENGTH_SHORT)
                            .show()
                          isSyncing = false
                        }
                      }
      ...
      Column(
        ...
        if (isSyncComplete) {
          Box(contentAlignment = Alignment.Center, modifier = Modifier.size(100.dp)) {
            CircularProgressIndicator(progress = { 1f }, modifier = Modifier.size(80.dp))
            Icon(
          ...
          Box(contentAlignment = Alignment.Center, modifier = Modifier.size(100.dp)) {
            CircularProgressIndicator(progress = { progressValue }, modifier = Modifier.size(80.dp))
            Text(
